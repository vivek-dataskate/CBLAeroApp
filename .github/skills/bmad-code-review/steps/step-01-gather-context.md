...
5a. **Load architecture and design standards for all review modes** (full or no-spec):
   - Locate and fully load the architecture document from `{planning_artifacts}` (`*architecture*.md`, whole file or all shards).
   - Locate and fully load the development standards document from `{planning_artifacts}` or workspace root (`*development-standards*.md` or `*design-standards*.md`).
   - These documents define: reusable capabilities that must not be reinvented, forbidden patterns, security requirements, coding conventions, and accepted library choices.
   - Reviewers must flag violations of architecture and development standards as `patch` or `decision_needed` findings.
   - Load ALL completed story files from `{implementation_artifacts}` that are referenced by or related to the current diff (by epic/story number or file overlap). These reveal implementation decisions and accepted patterns that the review must respect.
