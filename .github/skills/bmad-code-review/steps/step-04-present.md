...
#### Update architecture and development standards with new learnings

After all fixes have been applied and the story/sprint status has been updated, extract any learnings from this code review that should be persisted for future stories:

- **Architecture updates** (`{planning_artifacts}/*architecture*.md`): If the review revealed architectural gaps, undocumented patterns, new reusable capabilities introduced in this story, or corrections to existing architectural guidance — update the architecture document accordingly.
- **Development standards updates** (`{planning_artifacts}/*development-standards*.md` or `*design-standards*.md`): If the review found coding-convention violations, identified a missing standard, or confirmed a pattern that should be enforced going forward — add or update the relevant section in the development standards document.
- Document what was learned and why the change was made (one-line rationale per update).
- If no learnings are applicable, skip this step and note "No architecture/standards updates required."

Announce any documents updated:
> **Learnings captured:**
> - Architecture updated: <yes/no — brief description if yes>
> - Development standards updated: <yes/no — brief description if yes>
