...
After PR is created, always ask user how to merge:
...
Once user chooses (m/s/r), run:
```bash
gh pr merge <pr-number> --merge   # or --squash / --rebase
```

Then switch back to master, pull, and **delete the feature branch** both locally and remotely:
```bash
git checkout master
...
6. ✅ Ask: "Merge now? [m/s/r/n]" — merge using chosen strategy
7. ✅ After merge: checkout master, pull, delete feature branch locally and remotely
8. ✅ Report PR number, URL, and confirm branch deleted
...
After user picks [m/s/r]:
```
✅ PR #42 merged into master
✅ Switched to master, pulled latest
✅ Branch deleted: feature/story-2-7-global-scheduler (local + remote)
```
