...
<step n="2" goal="Load and analyze core artifacts">
  ...
  <!-- Architecture deep-load for reusable code and capability awareness -->
  <action>Fully load ALL architecture content (all sections, including sharded architecture files) — do not rely solely on previously cached summaries</action>
  **ARCHITECTURE CAPABILITY EXTRACTION:**
  - Identify ALL reusable services, utilities, helpers, and abstractions already built
  - Note capabilities that MUST be reused (do not reinvent)
  - Note capabilities that are explicitly NOT supported or should be avoided
  - Extract integration patterns, security requirements, and data-flow constraints applicable to this story

  <!-- All completed stories analysis for cross-story pattern learning -->
  <action>Scan {implementation_artifacts} for ALL completed story files (any story with Status: done, Status: review, or stories whose sprint-status entry is "done")</action>
  <action>Load ALL found completed story files — not just the immediately previous one</action>
  **ALL COMPLETED STORIES INTELLIGENCE:**
  - Patterns and conventions established across ALL past stories
  - Libraries, utilities, and abstractions introduced and how they are used
  - Testing approaches, file-naming conventions, and code-structure patterns
  - Mistakes, regressions, and corrections documented in Dev Agent Records
  - Reusable code components and helpers already built (do not rebuild)
  - Approaches that were tried and rejected and why
  <action>Extract all cross-story learnings that inform current story implementation</action>
