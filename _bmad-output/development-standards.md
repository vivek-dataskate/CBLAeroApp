...
- **Capture a real payload before writing the mapper.** Third-party webhooks (Clay, Zapier, custom partner integrations) often ship payload shapes that disagree with vendor docs. Point the webhook at webhook.site (or equivalent capture endpoint), fire one real row from the upstream system, and freeze the raw JSON as a test fixture **before** designing the mapper.
- **Freeze the real payload as a regression fixture.** The captured JSON must be committed to the test suite as an immutable fixture and exercised by the mapper test. This is the fastest way to catch silent vendor shape drift on future redeploys. See Story 2.8's `Michaela Ealey` fixture in [clay-mapper.test.ts](../src/modules/__tests__/clay-mapper.test.ts) for the pattern.
- **Build shape-tolerant mappers.** Never hard-code a single nested key path. Probe a small ordered fallback list of plausible keys (vendor rename, column alias, snake vs camel case) and preserve the full raw payload under `extra_attributes.<provider>.*` so future field promotion never needs reingestion. Mapper config (which nested key, which sidecar field) lives in env vars, not constants.
- **Push beats pull when the vendor offers an HTTP column.** Prefer receiving pushes on a `POST /api/webhooks/<provider>` endpoint over polling a cursor on the scheduler: no cursor state, no poll interval tuning, no rate-limit bookkeeping, and the vendor does the fanout. Only fall back to pull when the vendor exposes no push mechanism.
- **Bearer-token auth for webhooks.** Validate `Authorization: Bearer {PROVIDER}_WEBHOOK_SECRET` as the first operation in the handler, before reading the body. Startup must fail loudly if the secret env var is unset — no silent-accept fallback. Size-cap the body (Clay uses 256 KB) before JSON parsing.
- **Fail-loud env resolution for required external identities.** When a webhook stamps ingested rows with a resolved user/actor/team (e.g. `CLAY_DEFAULT_ASSIGNEE_EMAIL`), resolve on first use against the canonical table, cache for the process lifetime, and return HTTP 503 if the resolution fails. Never fall back to a default or orphan the row. See [clay route.ts `resolveAssigneeUserId`](../src/app/api/webhooks/clay/route.ts).
- **Provenance columns use preserve-if-set RPC merge, not application-level read-before-write.** When a column tracks "who first brought this row in" (e.g. `source_recruiter_actor_id`), set it via an `INSERT … ON CONFLICT DO UPDATE SET col = coalesce(target.col, excluded.col)` RPC. First write stamps; subsequent writes from any source preserve. Never read-then-write in application code — that's a race and a round-trip.
- **Hourly-bucket observability rows for high-volume push webhooks.** If a webhook can fire thousands of times in minutes (Clay fires per row), aggregate sync-run rows into one-per-hour buckets via a partial unique index scoped to that source alone (`WHERE source='<provider>'`). Invoke an atomic `INSERT … ON CONFLICT DO UPDATE SET accepted = accepted + excluded.accepted, …` RPC **once per request, not once per row**. Swallow observability RPC errors in the handler — observability must never block ingestion.
- **Debug-log the raw payload during initial rollout.** Add a `{PROVIDER}_WEBHOOK_DEBUG` env var (default `true` at launch) that dumps the first ~4k chars of each raw payload. Flip it off once the mapper is stable and the fixture suite is green on production traffic.

### Observability Table Mutations — Schema Changes Only in Migrations
- **Migrations MUST NOT `DELETE` or `UPDATE` rows in observability tables** — `sync_runs`, `sync_run_errors`, `content_fingerprints`, audit logs, scheduler run history. Schema changes (`ALTER TABLE`, `CREATE INDEX`, `CREATE OR REPLACE FUNCTION`, `CREATE TRIGGER`) are fine; row mutations are not.
- **Why:** During Story 2.8, a bundled "cleanup" `DELETE FROM sync_runs WHERE source='clay_enrichment'` wiped ~5,953 legitimate backfill rows when a real Clay backfill happened to run in parallel with the migration. Observability data is the authoritative evidence of what happened in production — once deleted, it cannot be reconstructed from the code.
- **How to apply:** If a cleanup is genuinely needed, it runs as a **separate, explicitly-invoked admin task** outside the deploy pipeline, with an auditable approval step. This rule is mirrored in [architecture.md §Observability Table Mutations](architecture.md).
