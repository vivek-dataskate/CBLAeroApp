...
- **Global scheduler (Story 2.7):** A single Render cron job (`CBLAero-Scheduler-Tick`) fires every 10 minutes, calling `POST /api/internal/jobs/run` which triggers `GlobalScheduler.runDueJobs()` + `processOutbox()`. The scheduler checks `next_run_at` for all 7 job definitions and dispatches only what's due. Per-job Render cron jobs (pre-2.7 pattern) are retired. Schedule cadences are code-defined in `registerIngestionJobs()` and synced to the DB on bootstrap — the admin UI displays them as read-only human-readable labels (e.g. "Every 15 minutes", "Daily at 2:00 AM UTC") with raw cron as a tooltip.
