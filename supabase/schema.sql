...
-- Story 2.7: Global scheduler control plane persistence
...
create table if not exists cblaero_app.schedule_runs (
  ...
  completed_at timestamptz,
  -- P7: constrained status values
  status text not null default 'pending' constraint schedule_runs_status_valid
...
-- DN2: Outbox events table for idempotent job dispatch
create table if not exists cblaero_app.outbox_events (
  ...
  payload jsonb,
  status text not null default 'pending' constraint outbox_events_status_valid
...
-- DN1: Atomic claim function using FOR UPDATE SKIP LOCKED
...
-- DN5: Seed policy registry for all ingestion schedule cadences
...
insert into cblaero_app.policy_versions (policy_id, value, effective_from, created_by_actor_id)
select r.id, jsonb_build_object('cron_expression', v.cron)::jsonb, now(), 'system'
from (values
...
where not exists (
  select 1 from cblaero_app.policy_versions pv where pv.policy_id = r.id
);
